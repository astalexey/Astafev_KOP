...
namespace BisinessLogic.FileModels
{
    ...
    {
        public static void CreateCustomerDocument(Info info)
        {
            using (SpreadsheetDocument spreadsheetDocument = SpreadsheetDocument.Create(info.FileName, SpreadsheetDocumentType.Workbook))
            {
                ...
                uint rowIndex = 2;
                foreach (var delivery in info.GetTechniques)
                {
                    InsertCellInWorksheet(new ExcelCellParameters
                    {
                        Worksheet = worksheetPart.Worksheet,
                        ...
                        ColumnName = "A",
                        RowIndex = rowIndex,
                        Text = "Компонент " + delivery.OrderName,
                        StyleIndex = 0U
                    });
                    InsertCellInWorksheet(new ExcelCellParameters
                    {
                        ...
                        ColumnName = "D",
                        RowIndex = rowIndex,
                        Text = " стоимостью " + delivery.OrderPrice, //Стоимость поставки
                        StyleIndex = 0U
                    });
                    InsertCellInWorksheet(new ExcelCellParameters
                    {
                        ...
                        ColumnName = "B",
                        RowIndex = rowIndex,
                        Text = " был доставлен " + delivery.GetTechniqueArrivalTime,
                        StyleIndex = 0U
                    });
                    rowIndex++;
                }
